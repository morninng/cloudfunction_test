...
const client_secret_mixideaproject_1 = require("./../secret/client_secret_mixideaproject");
...
class Calendar {
    constructor() {
        ...
        this.gettoken_executeAPI = (callback, data) => {
            console.log("gettoken_executeAPI");
            const token_ref = "/admin/google_token/";
            firebase_admin.database().ref(token_ref).once("value", (token_snapshot) => {
        ...
        this.update_googlecalendar_event = (data, auth) => {
            ...
            var calendar_event = {
                ...
                'description': "go to event page from here" +
                    "\n " + event_url +
                    "\n" +
            ...
            if (previous_calendar_event_id) {
                calendar.events.update({ auth: auth, calendarId: 'primary', resource: calendar_event, eventId: previous_calendar_event_id }, (err, calendar_event) => {
                    if (err) {
                ...
                calendar.events.insert({ auth: auth, calendarId: 'primary', resource: calendar_event }, (err, calendar_event) => {
                    if (err) {
        ...
        this.save_calendar_event_id = (event_id, calendar_event_id) => {
            const calendar_event_id_ref = "/event_related/calendar_event/" + event_id;
        ...
        this.create_newuser_calendar = (user_id, gmail_address) => {
            // 1.gmailのデータ追加、変更をsubscribe
            // 2. その人が参加している未来のイベントを抽出
            // 3. update_calendar_for_eventupdateを呼び出す。
        };
        this.get_oauth2Client = () => {
            if (!client_secret_mixideaproject_1.client_secret || !client_secret_mixideaproject_1.client_secret.installed) {
                console.log("slient_secret does not exist");
                return null;
            }
            var clientSecret = client_secret_mixideaproject_1.client_secret.installed.client_secret;
            var clientId = client_secret_mixideaproject_1.client_secret.installed.client_id;
            var redirectUrl = client_secret_mixideaproject_1.client_secret.installed.redirect_uris[0];
        ...
        this.listEvents_execute = (req, res, auth) => {
            var calendar = google.calendar('v3');
        ...
        this.update_calendar_for_user = (user_id) => {
            ...
            my_event_ref.orderByChild("date_time_start").startAt(retrieve_start_time).on("child_added", (snapshot) => {
                ...
                if (event_type == participate_1.ParticipateGoing || event_type == participate_1.ParticipateMaybe) {
